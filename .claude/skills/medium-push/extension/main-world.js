...
window.addEventListener('message', async event => {
  ...
  if (event.data.type === 'MEDIUM_PUSH_INSPECT') {
    window.postMessage(
      { type: 'MEDIUM_PUSH_INSPECT_RESULT', result: inspectDOM() },
      '*',
    )
    return
  }
  ...
  console.log(
    '[MediumPush] Received content, title:',
    title?.slice(0, 50),
    'html length:',
    html?.length,
  )
...
function findEditor() {
  return (
    document.querySelector(
      '.postArticle-content[role="textbox"][contenteditable="true"]',
    ) ||
...
function reportResult(success, method, error) {
  ...
  window.postMessage(
    { type: 'MEDIUM_PUSH_RESULT', success, method, error },
    '*',
  )
}
...
console.log(
  '[MediumPush] Main world script loaded (paste + zero-width-space mode)',
)
