...
这就是 SSR 状态管理陷阱。它是服务端渲染里最老的陷阱之一，几乎每个框架都给你留了口子。本文讲：模块级它怎么发生、[Pareto](https://github.com/childrentime/pareto) 的 `defineStore` 和 `defineContextStore` 怎么解决、以及选哪一个的决策规则。
...
拿不准，选 `defineContextStore`。它永远安全。
...
Pareto 的 `defineContextStore` 就是把这个模式做成了一等 API。`defineStore` 是全局单例快捷方式，给那些你已经确认过可以安全走这条路的场景用。
