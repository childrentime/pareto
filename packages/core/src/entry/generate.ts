...
export function generateServerEntry(options: {
  ...
  const routeDefs = routes.map((route) => {
    const optional = [
      ...
      route.headPaths?.length && `headPaths: ${JSON.stringify(route.headPaths.map(toImportPath))}`,
...
${optional}
  ...
  const cssImports = globalCssPaths.map(
    (p) => `import '${toImportPath(p)}';`,
  )
...
// Exported for SSG consumption
export const __routes = routes;
export const __moduleMap = moduleMap;
