...
To add cache headers, use [`configureServer`](/api/config/) in your `pareto.config.ts`:
...
- [Configuration](/api/config/) — `configureServer` and `configureVite` for customizing the build and server.
